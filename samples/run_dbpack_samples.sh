...
echo "../dist/dbpack start --config ../samples/go/config2.yaml 2>&1  | tee -a  ./logs/config2.log > ./logs/config2.log &"
../dist/dbpack start --config ../samples/go/config2.yaml 2>&1  | tee -a  ./logs/config2.log > ./logs/config2.log &

echo "../dist/dbpack start --config ../samples/go/config3.yaml 2>&1  | tee -a  ./logs/config3.log > ./logs/config3.log &"
